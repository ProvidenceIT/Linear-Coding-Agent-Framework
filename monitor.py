...
Features:
- Live progress tracking from Linear
                ...
                with open(log_file, 'r') as f:
                    for line in f:
                        try:
                            ...
                            if entry.get('category') == 'linear_api':
                                if entry.get('cached'):
                                    session_data['cached_calls'] += 1
                                else:
                                    session_data['linear_calls'] += 1
...
"""
            ...
            if 'project_url' in project_data:
                dashboard += f"\n  🔗 Linear: {project_data['project_url']}\n"
...
"""
            ...
            dashboard += f"  API Calls (last hour): {calls_last_hour}/1500\n"

            # API usage bar
            usage_pct = (calls_last_hour / 1500) * 100
            bar_length = 40
...
"""
            for i, session in enumerate(recent_sessions[:5], 1):
                ...
                dashboard += f"    Linear Calls: {session['linear_calls']} (Cached: {session['cached_calls']})\n"
