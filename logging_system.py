...
class StructuredLogger:
    ...
    def log_linear_api_call(
        self,
        operation: str,
        cached: bool = False,
        **metadata
    ):
        """Log Linear API call with caching info."""
        if cached:
            self.metrics['linear_api_cached'] += 1
            self.logger.info(
                f"Linear API (cached): {operation}",
                extra={
                    'category': 'linear_api',
                    'cached': True,
                    'operation': operation,
            ...
            self.metrics['linear_api_calls'] += 1
            self.logger.info(
                f"Linear API: {operation}",
                extra={
                    'category': 'linear_api',
                    'cached': False,
                    'operation': operation,
...
def create_logger(
    ...
    Usage:
        ...
        logger.log_issue_claimed("PRO-56", "Auth flow", priority=1)
